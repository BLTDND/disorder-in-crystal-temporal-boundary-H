...
phi_initial=phi_plus(k0,C_before0);
...
progressBar = waitbar(0, '进度');
disorder_max=100;
repeat_times=100;
psi_disorder=zeros(N_t,N,disorder_max);
for disorder_range=1:disorder_max
    waitbar(disorder_range/disorder_max, progressBar, sprintf('进度 %d/%d', disorder_range, disorder_max));
    for disorder_times=1:repeat_times
        
        %% construct random C
        C_before =4.4+(disorder_range-1)*(fibonacci(N)./max(fibonacci(N))-1)*(C_before0/disorder_times*2);%;(rand(1,N)-0.5)
        C_after  = -7+(disorder_range-1)*(fibonacci(N)./max(fibonacci(N))-1)*(C_after0/disorder_times*2);%.*linspace(1,1,N);(rand(1,N)-0.5)
        C=C_before;
        E=sqrt((2.*C*cos(k)).^2+kappa^2);
        vg_before=diff(E,k);
        vg_before0=double(subs(vg_before,k,pi/3));
    ...
    psi_disorder(:,:,disorder_range)=psi_disorder(:,:,disorder_range)./repeat_times;
...
level=1:disorder_max;
%% Energy T
for disorder_range=1:disorder_max
    saperate_point=find(psi_disorder(50,:,disorder_range)==max(psi_disorder(50,:,disorder_range)));
    %[~,saperate_point]=find(psi_disorder(:,tr_before,disorder_range)==max(psi_disorder(:,tr_before,disorder_range)));
    t_ratio(disorder_range)=sum(psi_disorder(end,1:saperate_point,disorder_range))/sum(psi_disorder(end,:,disorder_range));
end
...
figure(122)
order =15;
coefficients_t_ratio= polyfit(level, t_ratio, order);
xFit_t_ratio= linspace(min(level), max(level), 100);
yFit_t_ratio = polyval(coefficients_t_ratio, xFit_t_ratio);
plot(level,t_ratio,"*r")
hold on
plot(level,yFit_t_ratio,"-r")
title("transmission energy-disorder")



figure(8)
for disorder_range=1:disorder_max
    subplot(10,10,disorder_range),
    image(rescale(rot90(rot90(psi_disorder(1:2:end,:,disorder_range))),0,255));xlabel('omiga_m');ylabel('t');colorbar,title(strcat("disorder=",num2str(disorder_range)))
end

for disorder_range=1:disorder_max
    tr_before=find(psi_disorder(50,:,disorder_range)==max(psi_disorder(50,:,disorder_range)));
    Incident_angle(disorder_range)=pi/2-atan(abs(tr_before-N/2)/50);
...
% 绘制拟合曲线3
xFit_t_ratio= linspace(min(level), max(level), 100);
yFit_Refraction_angle = polyval(coefficients_Refraction_angle, xFit_t_ratio);
...
hold on
plot(xFit_t_ratio,yFit_Refraction_angle,"-g")% 
